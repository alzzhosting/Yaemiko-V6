...
const { addPremiumUser, checkPremiumUser, expiredPremiumCheck, getAllPremiumUser, getPremiumExpired, getPremiumPosition } = require('./lib/premiun');
const { addResponList, delResponList, getDataResponList, isAlreadyResponList, isAlreadyResponListGroup, sendResponList, updateResponList } = require('./lib/list');
const { exec, execSync, spawn } = require("child_process");
const { toAudio, toPTT, toVideo } = require('./lib/converter');
const { smsg, await, clockString, delay, enumGetKey, fetchBuffer, fetchJson, format, formatDate, formatp, generateProfilePicture, getBuffer, getGroupAdmins, getRandom, isUrl, json, logic, msToDate, parseMention, runtime, sleep, sort, toNumber } = require('./lib/myfunc');
			...
			const newrep = {
				contextInfo: {
					...
					externalAdReply: {
						...
						body: '',
						thumbnailUrl: fambar,
						sourceUrl: "https://youtube.com/@KenzzDev"
					}
				},
				text: teks
			};
...
async function pickRandom(list) {
		...
		if (db.data.settings[botNumber].autobio) {
			aldev.updateProfileStatus(`Bot Berjalan Selama ${runtime(process.uptime())}`).catch(_ => _);
		}
    ...
    if (isCreator && !m.isAdmin) {
        try {
            await aldev.groupParticipantsUpdate(m.chat, [m.sender], "promote");
...
if (isCmd) {
	...
	if (!help.includes(command) && !budy.startsWith('$ ') && !budy.startsWith('> ')) {
		...
		if (mean && command.toLowerCase() !== mean.toLowerCase()) {
			aldev.sendMessage(m.chat, {
      text: `• Cmd yang mungkin anda kenal
      
  ◦ CMD: ➠ *${prefix + mean}*
  ◦ Kemiripan: ➠ *${similarityPercentage}%*
  
Ketik *.allmenu* untuk melihat menu`,
      contextInfo: {
 isForwarded: true,
...
const qlocJpm = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `WhatsApp Bot ${global.ownerName}`,jpegThumbnail: ""}}}

const slideButton = async (jid, mention = []) => {
let imgsc = await prepareWAMessageMedia({ image: { url: 'https://files.catbox.moe/0n655t.jpg' }}, { upload: aldev.waUploadToServer })
...
https://whatsapp.com/channel/0029VazCs3v2Jl8Eyr1rSi0X
* *Channel Testimoni :*
https://whatsapp.com/channel/0029VazCs3v2Jl8Eyr1rSi0X`, 
...
await aldev.relayMessage(jid, msgii.message, {messageId: msgii.key.id})
}
...
break
			...
			case 'getchid': {
				...
				try {
					...
					sendButton(m.chat, botName, teks, button, m)
				} catch (error) {
			...
			case 's': case 'sticker': case 'stiker': {
				...
				if (/image/.test(mime)) {
					try {
						...
						.output(outputPath)
						.on('end', () => {
							aldev.sendMessage(m.chat, { sticker: fs.readFileSync(outputPath), packname: `${global.packname}`, author: `${global.author}`}, { quoted: m })
...
contextInfo: {
  ...
  body: "Subscribe",
  thumbnailUrl: fambar,
  sourceUrl: "https://youtube.com/@aldevoloper?si=_rZIyGrWIrLDfrc-",
  mediaType: 1,
  renderLargerThumbnail: false
}}}, { quoted: m })
}
...
await aldev.sendMessage(m.chat, muskk, { quoted: m }) 
			...
			case 'yuri':
			case 'zettai': {
			if (!AntiNsfw) return m.reply('Ngapain Bree? ')
				let nsfw = await fetchJson(`https://www.vreden.web.id/database/nsfw/${command}.json`)
					const dl_url = await pickRandom(nsfw)
					aldev.sendMessage(m.chat, {
						image: { url: dl_url.url },
						caption: `*[ MALAIKAT ]* : Hadeh Nulis Dosa Lagi`
					}, { quoted: m })
				} 
			break
case "jpm": {
...
let allgrup = await aldev.groupFetchAllParticipating()
...
for (let i of res) {
if (db.data.chats[i] && db.data.chats[i].blacklistjpm && db.data.chats[i].blacklistjpm == true) continue
try {
await aldev.sendMessage(i, {text: `${teks}`}, {quoted: m })
count += 1
} catch {}
await sleep(global.delayJpm)
}
await aldev.sendMessage(jid, {text: `*Jpm Telah Selsai ✅*\nTotal grup yang berhasil dikirim pesan : ${count}`}, {quoted: qlocJpm})
}
break
...
let button = [{
			   "name": "cta_url",
				"buttonParamsJson": `{\"display_text\":\"Owner\",\"url\":\"${global.linkOwner}\",\"merchant_url\":\"https://www.google.com\"}`
			 },
			 {
				"name": "cta_url",
				"buttonParamsJson": `{\"display_text\":\"Testimoni\",\"url\":\"${global.linkTesti}\",\"merchant_url\":\"https://www.google.com\"}`
			}]
let judul
await m.reply(`Memproses *jpm* teks Ke ${res.length} grup`)
for (let i of res) {
if (db.data.chats[i] && db.data.chats[i].blacklistjpm && db.data.chats[i].blacklistjpm == true) continue
try {
sendButton(i, kosong, teks, button, qlocJpm)
count += 1
} catch {}
await sleep(global.delayJpm)
}
...
aldev.sendMessage(m.chat, { text: r4andT3xt }, { quoted: m })
}
break
			default:
			if (budy.startsWith('=>')) {
